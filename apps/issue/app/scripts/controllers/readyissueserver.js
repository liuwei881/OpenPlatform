define(['angular'], function (angular) {
   ...
   */
  angular.module('issueApp.controllers.ReadyIssueServerCtrl', ['ui.bootstrap'])
    .controller('ReadyIssueServerCtrl', function ($scope, $state, $uibModal,Async,Sync) {
      this.awesomeThings = [
        'HTML5 Boilerplate',
        'AngularJS',
        'Karma'
        ...
        $scope.initPage = function(searchKey){
            Async.get('/api/v2/readyissue',{page: $scope.page, pageSize: $scope.pageSize, searchKey:searchKey}).
        ...
        $scope.Create = function () {
            ...
            modalInstance.save = function (item) {console.log(item);
                Async.save('/api/v2/readyissue/',item).
                    success(function (data) {
                        modalInstance.close();
                        $scope.initPage();
                    });
        ...
        $scope.Delete = function (i) {
           ...
           modalInstance.Delete = function (item) {
                Async.Delete('/api/v2/readyissue/',item).
                    success(function (data) {
                    console.log(item);
                        modalInstance.close();
                        $scope.initPage();
        ...
        $scope.pageAction = function (page) {
            Async.get('/api/v2/readyissue/',{page: page})
                .success(function (data) {
                    $scope.total = data.total;
                    $scope.rows = data.rows;
                });
