...
def get_datastores_info(host, user, pwd, port, datacenter):
    ...
    datastores_list = [k for k, v in datastores_info.items() if v["freespace"].endswith(
        "GB") and int(float(v["freespace"][space])) > 400 or v["freespace"].endswith("TB")]
...
def get_datastores_max(Host, User, Pwd, Port, datacenter):
    ...
    datacenter = get_obj(content, [vim.Datacenter], datacenter)
    ds_obj_list = datacenter.datastore
    datastores_max = {ds.summary.name: sizeof_fmt(
        ds.summary.freeSpace) for ds in ds_obj_list}
    datastores_max_new = {}
