...
from NgIssue.Entity.NgIssueModel import NgIssueServer
import json
from sqlalchemy import desc,or_,and_,engine


@urlmap(r'/ngissue\/?([0-9]*)')
class NgHandler(BaseHandler):
    ...
    def get(self, ident):
        ...
        totalquery = self.db.query(NgIssueServer.Id)
        NgIssueObj = self.db.query(NgIssueServer)
        if searchKey:
            totalquery = totalquery.filter(or_(NgIssueServer.DomainName.like('%%%s%%' % searchKey),NgIssueServer.HealthExam.like('%%%s%%' % searchKey),NgIssueServer.Port.like('%%%s%%' % searchKey)))
            NgIssueObj = NgIssueObj.filter(or_(NgIssueServer.DomainName.like('%%%s%%' % searchKey),NgIssueServer.HealthExam.like('%%%s%%' % searchKey),NgIssueServer.Port.like('%%%s%%' % searchKey)))
        self.Result['total'] = totalquery.count()
        serverTask = NgIssueObj.order_by(desc(NgIssueServer.VmwareId)).limit(pagesize).offset((page - 1) * pagesize).all()
