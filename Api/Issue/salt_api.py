...
url = 'https://10.96.141.112:8888'


def api_login():
    url = 'https://10.96.141.112:8888/login'
    ch = pycurl.Curl()
    ...
    ch.setopt(ch.POSTFIELDS, 'username=saltapi&password=liuwei1201&eauth=pam')
...
def get_result(cmd, arg, token=api_login(), tgt='*', expr_form='glob'):
    data = json.dumps([{"client":"local", "tgt":tgt, "fun":cmd,"arg":arg, 'expr_form':expr_form}])
...
if __name__ == "__main__":
    print(get_result("cmd.run", "ls", tgt="rancher_group", expr_form="nodegroup"))
